import { AracEkleDTO, RenkService, EnumsService, KazanilanAracDurumService, AracVersiyonService, AracDurumService, RuhsatSahibiService, LokasyonService } from '@zyazilim/online-ihale-data';
import { getSafeUrl } from '../../../../@core/utils/utils.service';

export class AracEkleExtendedDTO extends AracEkleDTO {

    hesaplanmisDegerleriAyarla(marka, model, versiyon) {
        this.aracAdi = `${marka} ${model} ${versiyon}`;
        this.seoBaslik = `İhale | ${marka} ${model} ${versiyon}`;
        this.seoMetaKeywords = `${marka} ${model} ${versiyon}`;
        this.seoMetaDescription = `${marka} ${model} ${versiyon}`;
        this.seoUrl = getSafeUrl(`${marka} ${model} ${versiyon}`);
    }
    static columnDefs(renkService: RenkService,
        ...
        lokasyonService: LokasyonService): any {
        return [
            {
                key: 'aracNo',
                name: 'Arac ID',
